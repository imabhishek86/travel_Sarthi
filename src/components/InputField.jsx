  ...
  return (
    <div className="mb-4">
      ...
      <div className={`relative group transition-all duration-300 ${isFocused ? 'scale-[1.02]' : 'scale-100'}`}>
        {/* Icon */}
        {icon && (
          <div className={`absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none transition-colors duration-300 ${isFocused ? 'text-primary' : 'text-gray-400'}`}>
            {icon}
          </div>
        )}
        ...
        <input
          ...
          className={`
            w-full rounded-xl border py-3 px-4 outline-none transition-all duration-300 bg-white/50 backdrop-blur-sm
            ${icon ? 'pl-10' : ''}
            ${isPassword ? 'pr-12' : ''}
            ${error 
      ...
      {/* Error Message */}
      <div className={`overflow-hidden transition-all duration-300 ${error ? 'max-h-6 opacity-100 mt-1' : 'max-h-0 opacity-0'}`}>
