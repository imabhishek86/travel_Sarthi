...
const Sidebar = ({ isMobileOpen, setIsMobileOpen }) => {
  ...
  const sidebarContent = (
    <div className="h-full flex flex-col bg-white dark:bg-gray-900 border-r border-gray-100 dark:border-gray-800 shadow-sm w-72 transition-colors duration-300">
      ...
      <div className="p-6 border-b border-gray-100 dark:border-gray-800 flex items-center gap-4">
        <img src={user.avatar} alt={user.name} className="w-12 h-12 rounded-full object-cover border-2 border-primary/20" />
        <div>
          <h3 className="font-bold text-dark dark:text-white truncate w-40">{user.name}</h3>
          <p className="text-xs text-gray-500 dark:text-gray-400 truncate w-40">{user.email}</p>
        </div>
      </div>
      ...
      <div className="p-4 border-t border-gray-100 dark:border-gray-800">
        <Link 
          to="/login"
          className="flex items-center gap-3 px-4 py-3.5 rounded-xl font-medium text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 transition-all duration-200"
        >
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
      ...
      {isMobileOpen && (
        <div className="fixed inset-0 z-50 lg:hidden flex">
          <div 
            ...
            onClick={() => setIsMobileOpen(false)}
          ></div>
          <div className="relative w-72 h-full bg-white dark:bg-gray-900 transform transition-transform ease-in-out duration-300">
            {sidebarContent}
          </div>
        </div>
      )}
